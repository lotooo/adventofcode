...
def solve(data):
  ...
  for j in range(len(data[0])):
    start.append(("v", (j, 0)))
    start.append(("^", (j, len(data) - 1)))
  for i in range(len(data)):
    start.append((">", (0, i)))
    start.append(("^", (len(data[0]) - 1, j)))
  for s in start:
    position, startup = s
    j, i = startup
    energy = [(j, i)]
    processed = []
    to_be_processed = [(position, (j, i))]
    while len(to_be_processed) > 0:
      node = to_be_processed.pop()
      direction = node[0]
      x, y = node[1]
      if x < 0 or x >= len(data[0]) or y < 0 or y >= len(data):
        # print(f"Escaping {n[1]}")
        continue
