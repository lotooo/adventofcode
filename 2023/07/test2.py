...
class Game:
  def __init__(self, game):
    ...
    # Unblock weird edge case for a hand full of Joker
    if self.hand == "JJJJJ":
      self.counter = Counter("AAAAA")
    while self.counter["J"] > 0:
      self.counter.subtract("J")
      if self.counter.most_common()[0][0] != "J":
        self.counter.update(self.counter.most_common()[0][0])
      else:
        self.counter.update(self.counter.most_common()[1][0])
    # Remove empty 'J's
    if "J" in self.counter and self.counter["J"] == 0:
      self.counter = Counter(self.counter.elements())

  @property
  def base14score(self):
    mapping = {"T": 10, "J": 0, "Q": 11, "K": 12, "A": 13}
    base10number = 0
