...
def best_first_search(grid, start):
    #start = grid.start
...
def solve(data):
    ...
    for item in grid.grid:
        node=grid.grid[item]
        if node.elevation == 'a':
            paths_distances = grid.search_distance(start=node)
            if len(paths_distances) > 0:
                # the starting point might not have a path ?
                solution1 = min(paths_distances) 
                results.append(solution1)

            #paths = grid.search(start=node)
            #paths_distances = list(map(lambda x: len(x), paths))
            #solution2 = min(paths_distances)-1 # Remove S
            #print(f"{solution2=}")

            #solution3 = best_first_search(grid, node)-1 # Remove S
            #print(f"{solution3=}")
